...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if (!HTTPScheme) {
        ...
        if ([urlString hasPrefix:@"weixin:"]) {
            urlString = [urlString stringByReplacingOccurrencesOfString:@"%3D" withString:@"="];
            url = [NSURL URLWithString:urlString];
        }
        BOOL bSucc = [[UIApplication sharedApplication] canOpenURL:url];
        if (!bSucc) {
            // 不可识别的，提示未安装客户端
            [XYWKTool openURLFromVc:self withUrl:url];
            // 禁止通过,当前请求
            decisionHandler(WKNavigationActionPolicyCancel);
            return;
        }
        ...
        if ([newUrl hasPrefix:@"https://wx.tenpay.com"]) {
            ...
            if ([backUrl isEqualToString:referer]) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    //设置授权域名
                    [request setValue:@"https://mi.yihu365.cn" forHTTPHeaderField:@"Referer"];
                    [self.webView loadRequest:request];
                });
